...
describe('AddAccountComponent', () => {
  ...
  it("Bank Name Check with value: 'Dummy bank Name'", () => {
    ...
    fixture.whenStable().then(() => {
      let test = component.ngForm.form.get('bankName');
      expect(test.valid).toBeTruthy();
    });
  });
  let randomValue = random(0, 1000000000);
  ...
  let randomValue2 = random(0, 1000000000);
  ...
  let randomValue3 = random(1000000000, 9999999999);
  it("1) Mobile Number Check with value<string> : '" + randomValue3 + "'", () => {
    component.mobileNumber = "" + randomValue3 + "";
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      let test = component.ngForm.form.get('mobileNumber');

        expect(test.valid).toBeTruthy();
      // fix this
    });
  });
  let randomValue4 = random(1000000000, 9999999999);
  ...
  let randomValue5 = random(10000000000, 99999999999999);
  ...
  for (let i = 100000000000; i <= 10000000000000000; i *= 10) {
    it("Account Number Check with value<string> ", () => {
      ...
      fixture.detectChanges();
      fixture.whenStable().then(() => {
        let test = component.ngForm.form.get('accountNumber');
        expect(test.valid).toBeTruthy();
      });
    });
  ...
  it("Ballance Accepts Digits", () => {
    ...
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      let test = component.ngForm.form.get('balance');
      expect(test.valid).toBeTruthy();
    });
  });
});
